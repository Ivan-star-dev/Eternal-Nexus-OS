...
- [x] ~~**Task C2:** Implement the `GeopoliticsMap.tsx` MapLibre shell using the new serverless `pmtiles://` registrar. Tie into the Tribunal data layer. (Owner tags: `@claude` + `@copilot`)~~

### 🎨 DESIGN & INTERFACE (The Flesh)
- [x] ~~**Task U1:** Apply the Dark Glassmorphism "Eternal Nexus" visual DNA directly to the Geopolitics Mapbox JSON styles. Hide unnecessary labels, prioritize glowing neon borders. (Owner tags: `@copilot` + `@ui`)~~
