...
- [x] **Task C3:** ~~Wire Atlas sensor data (climate, earthquake, pollution) through event bus.~~ ✅ DONE — `useRealtimeData` publishes `atlas.marker` events, `useNexusState.addAtlasData()` bridge. Gate 5b: 5 new tests. (Owner: `@claude`, PR #26)
- [x] **Task C4:** ~~Nervous System observability layer — bounded event ledger + dev inspector + deterministic replay.~~ ✅ DONE — `ledger.ts` ring buffer, `useEventLedger` hook, `EventInspector` dev overlay, 24 gate tests. Also fixed Tailwind v4 + tsconfig issues. (Owner: `@claude`, PR #26)
- [x] **Task C4b:** ~~Add geopolitics conflict tension heatmap layer to GeopoliticsMap — consume `RealtimeDataPoint` with `source: 'geopolitics'` and `conflictLevel` field, render as color-gradient fill layer (teal→orange→red).~~ ✅ DONE — Heatmap layer (teal→orange→red) in `GeopoliticsMap.tsx`, `ConflictGeoJSON` in `useGeopoliticsMap.ts`, `updateConflictData` exposed. (Owner: `@copilot`, PR #33)
...
- [x] ~~**Task U1:** Apply the Dark Glassmorphism "Eternal Nexus" visual DNA directly to the Geopolitics MapLibre styles. Base style is in `src/lib/map/dark-style.ts`. Add: neon border animations, label typography, hover states, custom verdict markers. (Owner tags: `@copilot` + `@ui`)~~ ✅ DONE — Triple-layer neon border glow, teal capital markers, gold country labels enabled, glassmorphism popups + nav controls, animated border pulse. (@copilot, this PR)
- [x] ~~**Task U2:** Build a MapLibre layer toggle panel for GeopoliticsMap — let users enable/disable conflict tension, migration routes, energy grid overlays. Follow EnvironmentPanel.tsx pattern from Atlas. (Owner: `@copilot`)~~ ✅ DONE — `LayerTogglePanel.tsx` with 4 toggle buttons (conflict heat, migration routes, energy grid, verdicts), dark glassmorphism styling, wired into `GeopoliticsMap.tsx`. (@copilot, this PR)
