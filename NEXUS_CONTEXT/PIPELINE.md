...
- [ ] **Task C5:** Implement event bus persistence layer — localStorage adapter, bounded capacity, idempotent rehydration, 18 gate tests. IN REVIEW — `src/lib/events/persistence.ts` in PR #51 (open, ready to merge). (Owner: `@claude`)
- [x] ~~**Task C5:** Implement event bus persistence layer — localStorage adapter, bounded capacity, idempotent rehydration, 18 gate tests.~~ ✅ DONE — `src/lib/events/persistence.ts`, PR #51 open + ready to merge. (Owner: `@claude`)
