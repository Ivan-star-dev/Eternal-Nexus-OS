...
- [x] ~~**Task C4:** Add geopolitics conflict tension heatmap layer to GeopoliticsMap — consume `RealtimeDataPoint` with `source: 'geopolitics'` and `conflictLevel` field, render as color-gradient fill layer (teal→orange→red).~~ ✅ DONE — `useConflictHeatmap.ts` hook + MapLibre heatmap + circle layers in `GeopoliticsMap.tsx`, seeded with 8 known hotspots, subscribes to `tribunal.verdict` events, wired to layer toggle panel. (Owner: `@claude` + `@copilot`)
