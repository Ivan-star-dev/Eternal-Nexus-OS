...
- [x] ~~**Task C4:** Add geopolitics conflict tension heatmap layer to GeopoliticsMap — consume `RealtimeDataPoint` with `source: 'geopolitics'` and `conflictLevel` field, render as color-gradient fill layer (teal→orange→red). (Owner: `@claude` + `@copilot`)~~ ✅ DONE — Heatmap layer (teal→orange→red) in `GeopoliticsMap.tsx`, `ConflictGeoJSON` in `useGeopoliticsMap.ts`, `updateConflictData` exposed. (@copilot, this PR)
