...
const mapDataCategory = (dp: RealtimeDataPoint): IndexCategory => {
  ...
  if (dp.co2 !== undefined || dp.temperature !== undefined || source === 'climate') {
    return 'climate';
  }
  if (source === 'economy') return 'economy';
  if (source === 'geopolitics') return 'security';
  if (source === 'energy') return 'infra';
  if (source === 'migration') return 'health';
  return 'climate';
};
