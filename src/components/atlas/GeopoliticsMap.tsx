 ...
 * PMTiles protocol: served serverlessly via @/lib/map/pmtiles-protocol
 *
 * Force-Task C2 (FORCE_TASK_ACTIVATION_002):
 *   - MapLibre GL JS shell activated (was commented-out stub)
 *   - PMTiles protocol adapter wired via registerPMTilesProtocol()
 *   - Dark glassmorphism style applied via createDarkStyle()
 *   - Tribunal data layer connected via useNexusState()
 *   - React overlay markers kept as fallback; migrate to MapLibre markers post-T-002
 */

import maplibregl from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";
import { registerPMTilesProtocol, deregisterPMTilesProtocol } from "@/lib/map/pmtiles-protocol";
import { createDarkStyle } from "@/lib/map/dark-style";
...
export function GeopoliticsMap({ className = "", onFeatureClick }: GeopoliticsMapProps) {
  ...
  useEffect(() => {
    ...
    // 2. Initialise the map with dark glassmorphism style
    const map = new maplibregl.Map({
      container: containerRef.current,
      style: createDarkStyle(),
      center: [0, 20],
      zoom: 1.8,
  ...
  return (
    ...
    >
      ...
      {/* React-layer markers wire the Atlas bus integration.                  */}
      {/* TODO (T-002): migrate to native MapLibre markers once useTribunalLayer() */}
      {/* is available — use map.getSource("tribunal-events").setData(...).    */}
